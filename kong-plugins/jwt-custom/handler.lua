...
local function load_credential(jwt_secret_key)
  local row, err = kong.db.jwt_custom_secrets:select_by_key(jwt_secret_key)
  if err then
    return nil, err
  end
  return row
...
local function do_authentication(conf)
  ...
  -- Retrieve the secret
  local jwt_secret_cache_key = kong.db.jwt_custom_secrets:cache_key(jwt_secret_key)
  ...
  local sub_claim = claims.sub
  if sub_claim then
      kong.service.request.set_header("X-User-ID", tostring(sub_claim))
      kong.log.debug("Added X-User-ID header with value: ", tostring(sub_claim))
  end
